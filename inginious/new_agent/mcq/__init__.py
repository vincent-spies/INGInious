...
from inginious.new_agent import Agent
...
from inginious.new_agent import GradingUnit

class MCQAgent(Agent):
    def __init__(self, db, name, tasks_filesystem):
        super().__init__(db, name, tasks_filesystem, {"mcq": self.MCQGradingUnit})
        ...
        languages = ["en", "fr"]
        self._translations = {
            lang: gettext.translation('messages', get_root_path() + '/agent/mcq_agent/i18n', [lang])
            for lang in languages
        }

    class MCQGradingUnit(GradingUnit):
