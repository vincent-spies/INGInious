...
class FacebookAuthMethod(AuthMethod):
    ...
    def get_auth_link(self, auth_storage, share=False):
        facebook = OAuth2Session(self._client_id, scope=scope + (["publish_actions"] if share else []), redirect_uri=webdict().home + self._callback_page)
    ...
    def callback(self, auth_storage):
        facebook = OAuth2Session(self._client_id, state=auth_storage["oauth_state"], redirect_uri=webdict().home + self._callback_page)
        try:
            ...
            r = facebook.post("https://graph.facebook.com/me/objects/website",
                              {
                                  "object": json.dumps({
                                      ...
                                      "og:url": webdict().home + "/course/" + course.get_id() + "/" + task.get_id(),
